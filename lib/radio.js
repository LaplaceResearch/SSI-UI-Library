...
$.fn.radio = function( options ) {
    ...
    var checkbox_template = '<div class="funkyradio-darkrose">' +
            ...
            '<label for="SIS-<%= item.id %>">' +
                '<% if(item.text_field_name){ %>' +
                    '<input type="text" id="<%= item.text_field_id %>" name="<%= item.text_field_name %>" value="<%= item.text_field_value %>" placeholder="<%= item.label %>">' +
                '<% }else{ %>' +
                    '<%= item.label %>' +
                '<% } %>' +
            '</label>' +
    ...
    $('tr.clickable').each(function(){
        ...
        if($(this).find('input[type="text"]').length) {
           ...
           data['text_field_id'] = $(this).find('input[type="text"]').attr('id');
    ...
    question_body.html('');
    question_body.append('<div class="funkyradio"></div>');
